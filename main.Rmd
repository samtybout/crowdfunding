...
optimal_goal = function(target, model, platform, max_ratio = 100, margin = 1){
  # Finds the goal that maximizes odds of raising at least as much money as the target
  low = 0
  high = target*max_ratio
  while(high-low > margin){
    mid = (low+high)/2
    lower = (low+mid)/2
    higher = (mid+high)/2
    p_lower = p_target(target, lower, model, platform)
    p_higher = p_target(target, higher, model, platform)
    print(paste(low, round(p_lower, 5), mid))
    print(paste(mid, round(p_higher, 5), high))
    print("")
    if(p_lower > p_higher){
      high = mid
    } else{
