...
This is a simplified summary of the analysis. For a more in-depth discussion with the source code included, go [here](summary_detailed.html).
...
```{r echo=FALSE}
print("Indiegogo Data")
glimpse(ig_data)
print("Kickstarter Data")
glimpse(ks_data)
```
...
```{r generalization, include=FALSE}
...
```{r category analysis, echo=FALSE}
...
The lines show the estimated probability of a project meeting its goal, and the dots show a sample of the data. Dots at the top are projects that met their funding goal and dots and the bottom are projects that missed their goal. The fit here suggests that projects with low goals have much better odds on Kickstarter.

#### Money raised

The second part of the model predicts how much money a project will raise, based on whether or not it meets its goal and what that goal is. I did this with a more computationally intensive method involving some Bayesian statistics.
...
```{r load saved results, include=FALSE}
...
After generating both components of the model, I was able to make predictions about project funding:
...
In general, it seems that the amount of money a project raises has little to do with to its stated goal. Kickstarter projects can raise small amounts of money more consistently, but the platform's all-or-nothing funding rule makes it much riskier than Indiegogo for projects with large goals.